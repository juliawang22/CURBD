...
def trainMultiRegionRNNCUDA(activity, dtData=1, dtFactor=1, g=1.5, tauRNN=0.01, tauWN=0.1, ampInWN=0.01, nRunTrain=2000, nRunFree=10, P0=1.0, nonLinearity=cp.tanh, nonLinearity_inv=cp.arctanh, resetPoints=None, plotStatus=True, verbose=True, regions=None):
    """
    Same as above but using cupy
    """
    if dtData is None:
        print('dtData not specified. Defaulting to 1.');
        dtData = 1;
...
# set up everything for training
    ...
    ampWN = math.sqrt(tauWN/dtRNN)
    iWN = ampWN * cp.random.randn(number_units, len(tRNN))
    inputWN = cp.ones((number_units, len(tRNN)))
    for tt in range(1, len(tRNN)):
        inputWN[:, tt] = iWN[:, tt] + (inputWN[:, tt - 1] - iWN[:, tt])*cp.exp(- (dtRNN / tauWN))
    inputWN = ampInWN * inputWN

    # initialize directed interaction matrix J
    J = g * cp.random.randn(number_units, number_units) / math.sqrt(number_units)
    J0 = J.copy()
    ...
    # get standard deviation of entire data
    stdData = cp.std(Adata[iTarget, :])

    # get indices for each sample of model data
    iModelSample = cp.zeros(len(tData), dtype=np.int32)
    for i in range(len(tData)):
